...
import { getBlogArticle, publicBlogArticles } from "@/content/blog";
...
export function generateStaticParams() {
  return publicBlogArticles.map((article) => ({
    slug: article.slug
  }));
}
...
export default async function BlogArticlePage({ params }: BlogArticlePageProps) {
  ...
  return (
    <main>
      ...
      <article className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          <Link href="/blog" className="inline-flex items-center gap-2 text-sm font-semibold text-ink underline">
            <ArrowLeft aria-hidden="true" size={16} /> Retour aux ressources
          </Link>
          <div className="mt-8">
          ...
          <section className="mt-12 rounded-md border border-stone-200 bg-white p-6">
            <h2 className="text-2xl font-semibold text-ink">Pour approfondir</h2>
          ...
          <section className="mt-10 rounded-md border border-gold/60 bg-sage p-6">
            <h2 className="text-2xl font-semibold text-ink">Votre paroisse rencontre ce besoin ?</h2>
            <p className="mt-3 leading-7 text-stone-650">
              Parlons de votre situation, de vos contraintes et de ce qui aiderait vraiment vos
              équipes comme vos fidèles.
            </p>
      ...
      <CTA title="Un site plus clair peut déjà changer beaucoup de choses." />
    </main>
  );
}
